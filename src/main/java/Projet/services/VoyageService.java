...
public class VoyageService implements IVoyageService<Voyage> {
    ...
    public Integer findVoyageIdByDepartAndDestination(String depart, String destination) {
        String query = "SELECT VID FROM voyage WHERE depart = ? AND destination = ?";
        try (PreparedStatement stmt = MyConnection.getInstance().getCnx().prepareStatement(query)) {
            ...
            if (rs.next()) {
                return rs.getInt("VID");  // Return the voyage ID directly
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return null;  // Return null if no voyage is found
    }

    public Voyage findVoyageById(int voyageId) {
        String query = "SELECT * FROM voyage WHERE VID = ?";
